...
const CreateOrders = () => {
    ...
    try {
        ...
        await orders.complete(createdOrder.id, `Продажа завершена. Покупатель: ${customerInfo.name}`);
    ...
    <div className="container">
        ...
        <div className="action-buttons">
            <button className="btn btn-outline" onClick={() => setShowCart(!showCart)}>Корзина ({getTotalItems()})</button>
        ...
        <div className="main">
            ...
            <div className="grid">
                ...
                <div key={item.id} className="card item-card">
                    ...
                    <button className="btn" onClick={() => addToCart(item)} disabled={item.current_stock === 0}>В корзину</button>
                    )}
                </div>
                ))}
            </div>
                ...
                <div className="checkout">
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            if (rentalId) {
                                ...
                                if (selectedRental) {
                                    ...
                                    // ДОБАВЛЕНО: Установка selectedProperty на основе выбранной аренды
                                    setSelectedProperty(selectedRental.property.id);
                                }
                            } else {
                                setCustomerInfo({ name: '', phone: '', email: '' });
                                // ДОБАВЛЕНО: Очистка selectedProperty, если аренда не выбрана
                                setSelectedProperty('');
                            }
                        }}
                        >
                        ...
                        {availableRentals.map(rental => (
                            ...
                            {rental.property.name} ({rental.property.number})
                            </option>
                        ))}
                    </select>
                            <div className="form-group">
                            <label htmlFor="property-select">Помещение для доставки:</label>
                            <select
                                id="property-select"
                                value={selectedProperty}
                                onChange={(e) => setSelectedProperty(e.target.value)}
                                disabled // ДОБАВЛЕНО: Блокируем, чтобы пользователь не мог его изменить вручную
                            >
