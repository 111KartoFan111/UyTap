...
class PayrollExtendedService:
    ...
    def create_payroll_template(
        ...
        # Деактивируем старые шаблоны для этого пользователя
        db.query(PayrollTemplate).filter(
            and_(
                PayrollTemplate.user_id == uuid.UUID(template_data.user_id),
                PayrollTemplate.status == PayrollTemplateStatus.ACTIVE
            )
        ).update({
            "status": PayrollTemplateStatus.INACTIVE,
    ...
    def _create_payroll_from_template(
        ...
        allowances_total = sum(template.automatic_allowances.values()) if template.automatic_allowances else 0
        
        # Применяем автоматические вычеты
        deductions_total = sum(template.automatic_deductions.values()) if template.automatic_deductions else 0
    ...
    def add_payroll_operation(
        ...
        operation = PayrollOperation(
            ...
            **operation_data.dict(exclude={'metadata'}),  # Исключаем metadata из dict()
            operation_metadata=operation_data.metadata,   # Устанавливаем отдельно
            created_at=datetime.now(timezone.utc),
            updated_at=datetime.now(timezone.utc)
        )
    ...
    def apply_recurring_operations(db: Session, organization_id: uuid.UUID):
        ...
        for original_op in recurring_operations:
            ...
            new_operation = PayrollOperation(
                ...
                operation_metadata=original_op.operation_metadata,
                is_applied=False
            )
    ...
    def get_payroll_forecast(
        ...
        for month_offset in range(1, forecast_months + 1):
            ...
            for template in active_templates:
                ...
                allowances = sum(template.automatic_allowances.values()) if template.automatic_allowances else 0
        ...
        return {
            ...
            "summary": {
                "min_monthly_cost": min(f["total_forecast"] for f in forecast_data) if forecast_data else 0,
                "max_monthly_cost": max(f["total_forecast"] for f in forecast_data) if forecast_data else 0,
                "avg_monthly_cost": sum(f["total_forecast"] for f in forecast_data) / len(forecast_data) if forecast_data else 0
            }
        }